...
# Slave to connect to i2c master
class I2CSlaveBFM:
    def __init__(self):
        self.addr = None
        self.state = "IDLE"
        ...
        while True:
            ...
            if (prev_scl == 0 and scl == 1):
                if (self.state == "ADDR"):
                    ...
                    self.state = "IDLE" # master has sampled the data send on the previous falling edge, go back to idle

            if (prev_scl == 1 and scl == 0): # falling edge
                if (self.state == "ACK"):
