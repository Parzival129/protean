// flash_ctrl: the persona switcher as a CPU peripheral.
// same erase/copy/verify/recover FSM as flash_switch.v, but triggered by a
// `start` pulse + `slot_in` from the SoC instead of the on-board buttons.
//   start=1 (one cycle) -> latch slot_in, copy that slot to boot, verify,
//   then pulse recfg_n (or fall back to the golden slot on a bad copy).
// busy stays high until it's back in IDLE.
    ...
    always @(posedge clk) begin
        ...
        case (state)
            IDLE: begin
                ...
                counter <= 2'd0;
                recovering <= 1'b0;
                if (start) begin          // CPU kicked us off
                    slot <= slot_in;
                    state <= E_WREN_KICK;
                end
            end
            ...
            DONE: begin
                cs <= 1'd1;
                if (rsum == vsum) begin
                    recfg_n <= 1'b0;          // good copy -> reload into the new persona
                end else if (!recovering) begin
                    recovering <= 1'b1;       // bad copy -> retry from the golden slot
                    eaddr <= 24'h000000;
                    blk <= 8'd0;
                    counter <= 2'd0;
                    state <= E_WREN_KICK;
                end
                // golden also failed: stay here (busy stays high), never reconfig
            end
