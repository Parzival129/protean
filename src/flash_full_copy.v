// full copy, scratch -> scratch so theres no way to brick it (0x200000 -> 0x300000)
// 1. erase the whole dest region with 64k block erase (0xD8)
// 2. copy it over 256 bytes at a time: read a page -> rbuf -> program the page
// 3. stream the whole dest back and add up a checksum to prove it landed
// LEDs 0,2,4 = checksums match (good), LEDs 1,3,5 = mismatch

module flash_full_copy(
    input wire clk,
    ...
    reg [23:0] eaddr = 24'h300000; // erase address, walks the dest block by block
    reg [23:0] saddr = 24'h200000; // source address to read from
    reg [23:0] daddr = 24'h300000; // dest address to program to
    reg [23:0] vaddr = 24'h300000; // dest address to verify from
    ...
    always @(posedge clk) begin
        ...
        case (state)
            ...
            DONE: begin
                cs <= 1'd1;
                if (rsum == vsum)
                    led <= ~6'b010101; // checksums match! LEDs 0,2,4
                else
